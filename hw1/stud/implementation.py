...
class StudentModel(Model):
    ...
    def __init__(self) -> None:
        ...
        #SAVE WEIGHT PERFORMING LOCALLY
        #I HAVE PROBLEMS BUILDING THE ENTIRE VOCABULARY OF GLOVE with 400.000 directly on server
        #HENCE LOCALLY IS CREATED THE DICTIONARY and the associated weight of GLOVE, then are stored in pumpy files
        # that are then actually loaded and used in the SERVER during the call bash test.sh data/dev.tsv

        
        #self.weight_tensor, self.Dictionary = generate_glove_voc()
        #np.save('Dict.npy', self.Dictionary)
        #np.save('Weight.npy', self.weight_tensor.numpy())

        #LOAD
        self.Dictionary = np.load('hw1/data/Dict_Glove200.npy',allow_pickle='TRUE').item()
        weight = np.load('hw1/data/Weight_Glove200.npy',allow_pickle='TRUE')
        ...
        self.Ge = GloveEmbedding(self.weight_tensor,self.Dictionary,True,True)
        self.model = LSTM_Glovo(embedding_dim, hidden_dim, vocab_size, tagset_size)
        ...
        self.PATH = 'model/model_glove200.pt'
...
class Search():
    ...
    def explore(self, batch):
        ...
        for x in batch:
            ...
            #print(flipped)
            list_max,list_,counter,list_index,list_max_index=recursive_call(flipped,0,flipped.size()[0],0,[],[],[],[],cond,0,index_map,index_inv_map,hard_const)
...
class LSTM_Glovo(torch.nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size):
        ...
        self.lstm_glovo = torch.nn.LSTM(embedding_dim, hidden_dim, batch_first=True,num_layers=2,bidirectional=True,dropout=0.3)
    ...
    def forward(self, sentence):
        ...
        lstm_out, _ = self.lstm_glovo(sentence)
